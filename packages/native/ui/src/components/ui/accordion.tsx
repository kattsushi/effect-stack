...
function AccordionItem({
  ...
  return (
    <Animated.View className={'overflow-hidden'} layout={LinearTransition.duration(200)}>
      <AccordionPrimitive.Item className={cn('border-border border-b', className)} value={value} {...props} />
    </Animated.View>
  )
}
...
function InnerContent({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <Animated.View className={cn('pb-4', className)} entering={FadeIn} exiting={FadeOutUp.duration(200)}>
      {children}
    </Animated.View>
  )
}
