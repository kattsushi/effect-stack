...
describe('authentication', () => {
  ...
  effect('when user is not authenticated', () =>
    Effect.gen(function* () {
      ...
      const exit = yield* c.query(api.auth.getUserIdentity, {}).pipe(Effect.exit)

      assertFailure(exit, Cause.die(Runtime.makeFiberFailure(Cause.fail(new NoUserIdentityFoundError()))))
    }),
  )
})
