...
import type { DocumentDecodeError, DocumentEncodeError } from '../../src/server/database'
import { api } from './_generated/api'
import { ConfectDatabaseWriter, ConfectScheduler, confectAction, confectMutation } from './confect'
...
export const scheduledInsert = confectMutation({
  ...
  returns: Schema.Null,
  handler: ({ text }): Effect.Effect<null, DocumentEncodeError | DocumentDecodeError, ConfectDatabaseWriter> =>
    Effect.gen(function* () {
...
export const insertAt = confectAction({
  ...
  returns: Schema.Null,
  handler: ({ text, timestamp }): Effect.Effect<null, never, ConfectScheduler> =>
    Effect.gen(function* () {
      const scheduler = yield* ConfectScheduler
