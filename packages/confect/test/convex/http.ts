...
const ApiGroup = HttpApiGroup.make('apiGroup').add(
  HttpApiEndpoint.get('get', '/get').addSuccess(Schema.Literal('Hello, world!')).annotate(OpenApi.Title, 'Get'),
)
...
const ApiGroupPathPrefix = HttpApiGroup.make('apiGroupPathPrefix')
  .add(HttpApiEndpoint.get('get', '/get').addSuccess(Schema.Literal('Hello, world!')).annotate(OpenApi.Title, 'Get'))
  .prefix('/path-prefix')

class ApiPathPrefix extends HttpApi.make('ApiPathPrefix').add(ApiGroupPathPrefix) {}

const ApiGroupPathPrefixLive = HttpApiBuilder.group(ApiPathPrefix, 'apiGroupPathPrefix', (handlers) =>
  handlers.handle('get', () => Effect.succeed('Hello, world!' as const)),
)

const ApiPathPrefixLive = HttpApiBuilder.api(ApiPathPrefix).pipe(Layer.provide(ApiGroupPathPrefixLive))
