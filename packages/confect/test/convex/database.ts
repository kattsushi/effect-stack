...
import { ConfectDatabaseReader, ConfectDatabaseWriter, confectMutation, confectQuery } from './confect'
...
export const getByIndex = confectQuery({
  ...
  handler: ({ name, role, text }) =>
    Effect.gen(function* () {
      ...
      return yield* reader.table('notes').get('by_name_and_role_and_text', name, role, text)
    }),
})
...
export const paginate = confectQuery({
  ...
  handler: ({ cursor, numItems }) =>
    Effect.gen(function* () {
      ...
      return yield* reader.table('notes').index('by_creation_time').paginate({ cursor, numItems })
    }),
})
...
export const systemQuery = confectQuery({
  ...
  handler: () =>
    Effect.gen(function* () {
      ...
      return yield* reader.table('_storage').index('by_creation_time').collect()
    }),
})
