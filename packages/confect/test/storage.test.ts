...
import { assertEquals, assertFailure, assertInstanceOf } from '@effect/vitest/utils'
  ...
  describe('getUrl', () => {
    effect('when file exists', () =>
      Effect.gen(function* () {
        ...
        const urlString = yield* c.action(api.storage.confectStorageReaderGetUrl, {
          id,
        })
    ...
    effect('when file no longer exists', () =>
      Effect.gen(function* () {
        ...
        assertFailure(exit, Cause.die(Runtime.makeFiberFailure(Cause.fail(new FileNotFoundError({ id })))))
      }),
    )
  })
})
...
describe('ConfectStorageWriter', () => {
  effect('generateUploadUrl', () =>
    Effect.gen(function* () {
      ...
      const urlString = yield* c.action(api.storage.confectStorageWriterGenerateUploadUrl)
      const url = yield* Schema.decode(Schema.URL)(urlString)
  ...
  describe('get', () => {
    effect('when file exists', () =>
      Effect.gen(function* () {
        ...
        const blobSize = yield* c.action(api.storage.confectStorageActionWriterGet, {
          id,
        })
  ...
  effect('store', () =>
    Effect.gen(function* () {
      ...
      const blobText = yield* c.run(({ storage }) =>
        storage.get(id).then((blob) => (blob ? blob.text() : Promise.resolve(null))),
      )
