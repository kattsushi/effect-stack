...
import { DocumentDecodeError, GetByIdFailure, GetByIndexFailure } from '../src/server/database'
  ...
  describe('getById', () => {
    ...
    effect('when document no longer exists', () =>
      Effect.gen(function* () {
        ...
        const noteId = yield* c.run(({ db }) => db.insert('notes', { text: 'Hello, world!' }))
    ...
    effect('when document is invalid', () =>
      Effect.gen(function* () {
        ...
        const noteId = yield* c.run(({ db }) => db.insert('notes', { text: invalidText }))
  ...
  describe('getByIndex', () => {
    ...
    effect('when document no longer exists', () =>
      Effect.gen(function* () {
        ...
        const noteId = yield* c.run(({ db }) => db.insert('notes', { author: { name, role }, text }))
  ...
  effect('first', () =>
    Effect.gen(function* () {
      ...
      const [noteId1, _noteId2] = yield* c.run(({ db }) =>
        Promise.all([db.insert('notes', { text: '1' }), db.insert('notes', { text: '2' })]),
      )

      const note = yield* c
        .query(api.database.first, {})
        .pipe(
          Effect.andThen(Schema.decode(Schema.Option(confectSchema.tableSchemas.notes.withSystemFields))),
  ...
  describe('withIndex', () => {
    effect('without query range without order', () =>
      Effect.gen(function* () {
        ...
        const notes = yield* c.query(api.database.withIndexWithQueryRangeWithOrder)
    ...
    effect('with query range without order', () =>
      Effect.gen(function* () {
        ...
        const notes = yield* c.query(api.database.withIndexWithQueryRangeWithoutOrder, {
          text: '2',
        })
    ...
    effect('with query range and order', () =>
      Effect.gen(function* () {
        ...
        const notes = yield* c.query(api.database.withIndexWithQueryRangeAndOrder, {
          text: '1',
        })
    ...
    effect('without query range with order', () =>
      Effect.gen(function* () {
        ...
        const notes = yield* c.query(api.database.withIndexWithoutQueryRangeWithOrder)
    ...
    effect('without query range without order', () =>
      Effect.gen(function* () {
        ...
        const notes = yield* c.query(api.database.withIndexWithoutQueryRangeWithoutOrder)
    ...
    describe('system tables', () => {
      effect('get', () =>
        Effect.gen(function* () {
          ...
          const storageId = yield* c.run(({ storage }) => storage.store(new Blob(['Hello, world!'])))
...
describe('ConfectDatabaseWriter', () => {
  ...
  effect('replace', () =>
    Effect.gen(function* () {
      ...
      const noteId = yield* c.run(({ db }) => db.insert('notes', { text: initialText }))
