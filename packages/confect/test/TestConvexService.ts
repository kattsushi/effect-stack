...
import { convexTest, type TestConvexForDataModel, type TestConvexForDataModelAndIdentity } from 'convex-test'
...
export type TestConvexService = {
  withIdentity: (userIdentity: Partial<UserIdentity>) => TestConvexServiceWithoutIdentity
} & TestConvexServiceWithoutIdentity

export const TestConvexService = Context.GenericTag<TestConvexService>('@rjdellecese/confect/TestConvexService')

class TestConvexServiceImplWithoutIdentity implements TestConvexServiceWithoutIdentity {
  constructor(private testConvex: TestConvexForDataModel<DataModelFromSchemaDefinition<typeof schema>>) {}

  readonly query = <Query extends FunctionReference<'query', any>>(query: Query, ...args: OptionalRestArgs<Query>) =>
  ...
  readonly fetch = <PathQueryFragment extends string>(pathQueryFragment: PathQueryFragment, init?: RequestInit) =>
...
class TestConvexServiceImpl implements TestConvexService {
  constructor(private testConvex: TestConvexForDataModelAndIdentity<DataModelFromSchemaDefinition<typeof schema>>) {
    this.testConvex = testConvex
  }

  readonly withIdentity = (userIdentity: Partial<UserIdentity>) =>
    new TestConvexServiceImplWithoutIdentity(this.testConvex.withIdentity(userIdentity))
