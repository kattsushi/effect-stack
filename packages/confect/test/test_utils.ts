...
) => Effect.Effect<E, A, TestServices.TestServices | TestConvexService.TestConvexService>
type Timeout<A, E> = Parameters<typeof effect_<E, A>>[2]

export const effect = <A, E>(name: Name<A, E>, self: Self<A, E>, timeout?: Timeout<A, E>) => {
  type Ctx = Parameters<Self<A, E>>[0]
  type Eff = ReturnType<Self<A, E>>

  const self_ = (
    ctx_: Ctx,
  ): Effect.Effect<Effect.Effect.Success<Eff>, Effect.Effect.Error<Eff>, TestServices.TestServices> =>
