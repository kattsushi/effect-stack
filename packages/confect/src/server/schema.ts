...
import { compileTableSchema, type TableSchemaToTableValidator } from './schema_to_validator'
...
const tableSchemasFromConfectSchema = <ConfectSchema extends GenericConfectSchema>(
 ...
 */
export type GenericConfectSchema = Record<string, GenericConfectTableDefinition>

/**
 * A Confect schema definition tracks the Confect schema, its Convex schema definition, and all of its table schemas.
 */
export type GenericConfectSchemaDefinition = ConfectSchemaDefinition<GenericConfectSchema>

export interface ConfectSchemaDefinition<ConfectSchema extends GenericConfectSchema> {
  confectSchema: ConfectSchema
  ...
  implements ConfectSchemaDefinition<ConfectSchema>
{
  ...
  constructor(confectSchema: ConfectSchema) {
    ...
    this.convexSchemaDefinition = pipe(
      ...
      defineConvexSchema,
    ) as SchemaDefinition<SchemaDefinitionFromConfectSchemaDefinition<ConfectSchema>, true>
...
export const defineConfectSchema = <ConfectSchema extends GenericConfectSchema>(
  confectSchema: ConfectSchema,
): ConfectSchemaDefinition<ConfectSchema> => new ConfectSchemaDefinitionImpl<ConfectSchema>(confectSchema)

export type GenericConfectTableDefinition = ConfectTableDefinition<any, any, any, any, any>

export interface ConfectTableDefinition<
  TableSchema extends Schema.Schema.AnyNoContext,
  TableValidator extends GenericValidator = TableSchemaToTableValidator<TableSchema>,
...
> {
  ...
  >(
    ...
    TableSchema,
    TableValidator,
    Expand<Indexes & Record<IndexName, [FirstFieldPath, ...RestFieldPaths, IndexTiebreakerField]>>,
    SearchIndexes,
    VectorIndexes
  >
  searchIndex<
...
export type ConfectSchemaFromConfectSchemaDefinition<ConfectSchemaDef extends GenericConfectSchemaDefinition> =
  ConfectSchemaDef extends ConfectSchemaDefinition<infer ConfectSchema> ? ConfectSchema : never

/**
 * @ignore
 */
export type ConfectDataModelFromConfectSchemaDefinition<ConfectSchemaDef extends GenericConfectSchemaDefinition> =
...
class ConfectTableDefinitionImpl<
  TableSchema extends Schema.Schema.AnyNoContext,
  TableValidator extends Validator<any, any, any> = TableSchemaToTableValidator<TableSchema>,
  Indexes extends GenericTableIndexes = {},
  SearchIndexes extends GenericTableSearchIndexes = {},
  VectorIndexes extends GenericTableVectorIndexes = {},
> implements ConfectTableDefinition<TableSchema, TableValidator, Indexes, SearchIndexes, VectorIndexes>
{
  tableDefinition: TableDefinition<TableValidator, Indexes, SearchIndexes, VectorIndexes>
  ...
  constructor(tableSchema: TableSchema, tableValidator: TableValidator, indexes: Indexes = {} as Indexes) {
 ...
 */
export const defineConfectTable = <TableSchema extends Schema.Schema.AnyNoContext>(
  tableSchema: TableSchema,
): ConfectTableDefinition<TableSchema> => {
  const tableValidator = compileTableSchema(tableSchema)
  return new ConfectTableDefinitionImpl(tableSchema, tableValidator) as ConfectTableDefinition<TableSchema>
}

export type TableNamesInConfectSchema<ConfectSchema extends GenericConfectSchema> = keyof ConfectSchema & string

export type TableNamesInConfectSchemaDefinition<ConfectSchemaDefinition extends GenericConfectSchemaDefinition> =
  TableNamesInConfectSchema<ConfectSchemaDefinition['confectSchema']>

/**
 * Produce a Confect data model from a Confect schema.
 */
export type ConfectDataModelFromConfectSchema<ConfectSchema extends GenericConfectSchema> = {
  [TableName in keyof ConfectSchema & string]: ConfectSchema[TableName] extends ConfectTableDefinition<
    infer TableSchema,
...
type ExtractConfectDocument<TableName extends string, TableSchema extends Schema.Schema.AnyNoContext> = Expand<
  Readonly<IdField<TableName>> & Readonly<SystemFields> & TableSchema['Type']
>

type ExtractEncodedConfectDocument<TableName extends string, TableSchema extends Schema.Schema.AnyNoContext> = Expand<
  Readonly<IdField<TableName>> & Readonly<SystemFields> & TableSchema['Encoded']
>

export const confectSystemSchema = {
  _scheduled_functions: defineConfectTable(confectSystemTableSchemas._scheduled_functions),
  _storage: defineConfectTable(confectSystemTableSchemas._storage),
}

export type ConfectSystemSchema = typeof confectSystemSchema

export const confectSystemSchemaDefinition = defineConfectSchema(confectSystemSchema)

type ConfectSystemSchemaDefinition = typeof confectSystemSchemaDefinition

export type ConfectSystemDataModel = ConfectDataModelFromConfectSchemaDefinition<ConfectSystemSchemaDefinition>

export const extendWithConfectSystemSchema = <ConfectSchema extends GenericConfectSchema>(
  confectSchema: ConfectSchema,
): ExtendWithConfectSystemSchema<ConfectSchema> => ({ ...confectSchema, ...confectSystemSchema }) as any

export type ExtendWithConfectSystemSchema<ConfectSchema extends GenericConfectSchema> = Expand<
...
export type TableSchemasFromConfectDataModel<ConfectDataModel extends GenericConfectDataModel> = {
 ...
 * @public
 */
type ExtractDocument<TableName extends string, T extends Validator<any, any, any>> = Expand<
  IdField<TableName> & SystemFields & T['type']
> //the table name) and trick TypeScript into expanding them. // Add the system fields to `Value` (except `_id` because it depends on
