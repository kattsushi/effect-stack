...
): Validator<any, any, any> => runSyncThrow(compileAst(Schema.encodedSchema(schema).ast))
 ...
 */
export type TableSchemaToTableValidator<TableSchema extends Schema.Schema.AnyNoContext> = ValueToValidator<
  TableSchema['Encoded']
> extends infer Vd extends VObject<any, any, any, any> | VUnion<any, any, any, any>
  ? Vd
  : never

export const compileTableSchema = <TableSchema extends Schema.Schema.AnyNoContext>(
  schema: TableSchema,
...
type ArrayValueToValidator<Vl extends ReadonlyArray<ReadonlyValue>> = Vl extends ReadonlyArray<
  infer El extends ReadonlyValue
>
...
export type UndefinedOrValueToValidator<Vl extends ReadonlyValue | undefined> = undefined extends Vl
  ? [Vl] extends [(infer Val extends ReadonlyValue) | undefined]
    ? ValueToValidator<Val> extends infer Vd extends Validator<any, OptionalProperty, any>
      ? VOptional<Vd>
      : never
    : never
  : Vl extends ReadonlyValue
...
type UnionValueToValidator<Vl extends ReadonlyValue> = [Vl] extends [ReadonlyValue]
  ? IsUnion<Vl> extends true
    ? UnionToTuple<Vl> extends infer VlTuple extends ReadonlyArray<ReadonlyValue>
...
type ValueTupleToValidatorTuple<VlTuple extends ReadonlyArray<ReadonlyValue>> = VlTuple extends
  ...
  ? ValueTupleToValidatorTuple<VlRest> extends infer VdRest extends Validator<any, any, any>[]
    ...
    : never
  : VlTuple extends [infer Vl extends ReadonlyValue, ...infer VlRest extends ReadonlyArray<ReadonlyValue>]
...
export const compileSchema = <T, E>(schema: Schema.Schema<T, E>): ValueToValidator<(typeof schema)['Encoded']> =>
...
export const isRecursive = (ast: SchemaAST.AST): boolean =>
  pipe(
    ...
    Match.tag('Union', ({ types }) => Array.some(types, (type) => isRecursive(type))),
    ...
    Match.tag(
      ...
      ({ elements: optionalElements, rest: elements }) =>
        Array.some(optionalElements, (optionalElement) => isRecursive(optionalElement.type)) ||
  ...
  Effect.gen(function* () {
    const validatorEffects = isOptionalPropertyOfTypeLiteral
      ? Array.filterMap([first, second, ...rest], (type) =>
          Predicate.not(SchemaAST.isUndefinedKeyword)(type) ? Option.some(compileAst(type)) : Option.none(),
        )
...
const handleTupleType = ({ elements, rest }: SchemaAST.TupleType) =>
  Effect.gen(function* () {
    ...
    const arrayItemsValidator = yield* f === undefined
      ? pipe(
          restValidator,
          Effect.catchTag('NoSuchElementException', () => Effect.fail(new EmptyTupleIsNotSupportedError())),
        )
      : s === undefined
...
const handlePropertySignatures = (typeLiteralAst: SchemaAST.TypeLiteral) =>
  pipe(
    ...
    Effect.forEach(({ type, name, isOptional }) => {
      ...
      return Effect.fail(new UnsupportedPropertySignatureKeyTypeError({ propertyKey: name }))
    }),
    Effect.andThen((propertyNamesWithValidators) =>
      pipe(
        propertyNamesWithValidators,
        Array.reduce({} as Record<string, Validator<any, any, any>>, (acc, { propertyName, validator }) => ({
