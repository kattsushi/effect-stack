 ...
 *
 * This class analyzes Confect functions and their error schemas to generate
 * TypeScript type definitions that provide type safety for error handling.
 *
 * @since 1.0.0
 * @example
 * ```typescript
 * const generator = new ErrorTypesGenerator(functions, './types.d.ts', typeDefinitions, './convex')
...
export class ErrorTypesGenerator {
  ...
  /**
   * Creates a new ErrorTypesGenerator instance.
   *
  ...
  /**
   * Generates the TypeScript error types file.
   *
   * Creates type definitions for all Confect functions and their error schemas,
   * writes the content to the specified output path, and updates .gitignore.
   *
   * @since 1.0.0
   */
  generate(): void {
...
`
   ...
   *
   * Combines header, imports, module augmentation, helper types, and global
   * declarations into a single TypeScript definition file.
   *
   * @returns Complete TypeScript definition file content
   * @since 1.0.0
   * @private
   */
